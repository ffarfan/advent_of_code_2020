...
def puzzle_1(input_data):
    source_code = [line.split(' = ') for line in input_data]
    memory = {}

    bitmask = ''
    for operation, value in source_code:
        if operation == 'mask':
            bitmask = value
        else:
            mem_pos = int(operation[4: -1])
...
def puzzle_2(input_data):
    ...
    for operation, value in source_code:
        if operation == 'mask':
            ...
            mem_pos = int(operation[4: -1])
            for mask in get_all_masks(bit_mask):
                mem_pos = apply_mask(mem_pos, mask)
                memory[mem_pos] = int(value)
...
def apply_mask(value, mask):
    n_value = int(value)
    n_value |= int(mask.replace('X', '0'), 2)
    n_value &= int(mask.replace('X', '1'), 2)
...
if __name__ == '__main__':
    input_data = advent_utils.load_input_from_file('inputs/input_14.txt')
    # input_data = advent_utils.load_input_from_file('inputs/test_14_1.txt')
    # input_data = advent_utils.load_input_from_file('inputs/test_14_2.txt')
