...
def puzzle_1(input_data):
    return recite_number(input_data, 2020)


def puzzle_2(input_data):
    return recite_number(input_data, 30000000)


def recite_number(numbers, limit):
    spoken_numbers = defaultdict(list)

    for i, x in enumerate(numbers):
        spoken_numbers[x].append(i)

    while len(numbers) != limit:
        value = numbers[-1]
        _list = spoken_numbers[value]
        if len(_list) <= 1:
            numbers.append(0)
        else:
            difference = spoken_numbers[value][-1] - spoken_numbers[value][-2]
            numbers.append(difference)

        spoken_numbers[numbers[-1]].append(len(numbers) - 1)

    return numbers[-1]


if __name__ == '__main__':
    input_data = [1, 0, 16, 5, 17, 4]
    # input_data = [0, 3, 6]
