...
def puzzle_1(input_data, window_size):
    window = input_data[0: window_size]

    for number in input_data[window_size:]:
        if not is_valid_number(number, window):
            return number
...
def puzzle_2(input_data, invalid_number):
    slices = [input_data[s: e] for s, e in combinations(range(len(input_data) + 1), 2) if s != e]
...
def is_valid_number(number, window):
    two_nums = (list(permutations(window, 2)))
    result = [i for i in two_nums if i[0] + i[1] == number]

    return len(result) > 0


def load_numbers(input_data):
    return [int(line) for line in input_data]


if __name__ == '__main__':
    input_data = advent_utils.load_input_from_file('inputs/input_09.txt')
    window_size = 25
    # input_data = advent_utils.load_input_from_file('inputs/test_09.txt')
    # window_size = 5

    input_numbers = load_numbers(input_data)

    invalid_number = puzzle_1(input_numbers, window_size)

    print('puzzle_1: {s}'.format(s=invalid_number))
    print('puzzle_2: {s}'.format(s=puzzle_2(input_numbers, invalid_number)))
