...
def puzzle_1(floor_map):
    floor_map = get_final_floor_map(floor_map, iterate_floor_1)
    return flatten(floor_map).count('#')


def puzzle_2(floor_map):
    floor_map = get_final_floor_map(floor_map, iterate_floor_2)
    return flatten(floor_map).count('#')


def get_final_floor_map(floor_map, iterate_callback):
    iterate = True
    iteration = 0
    while iterate:
        new_floor_map = iterate_callback(floor_map)

        if flatten(new_floor_map) == flatten(floor_map):
            iterate = False
...
def iterate_floor_1(floor_map):
    new_floor_map = deepcopy(floor_map)

    for i in range(len(floor_map)):
        for j in range(len(floor_map[i])):
            seat = floor_map[i][j]
            occupied_neighbors = get_occupied_neighbors(floor_map, i, j)
...
def iterate_floor_2(floor_map):
    new_floor_map = deepcopy(floor_map)
    n, m = len(floor_map), len(floor_map[0])
        ...
        for j, seat in enumerate(row):
            ...
            for dx in range(-1, 2):
                for dy in range(-1, 2):
                    ...
                    nx = i + dx
                    ny = j + dy
                    while 0 <= nx < n and 0 <= ny < m and floor_map[nx][ny] == '.':
                        nx += dx
                        ny += dy

                    if 0 <= nx < n and 0 <= ny < m:
                        occupied_neighbors += floor_map[nx][ny] == '#'
...
def get_occupied_neighbors(floor_map, x, y):
    ...
    for xx in range(x - 1, x + 2):
        for yy in range(y - 1, y + 2):
            if 0 <= xx < len(floor_map) and 0 <= yy < len(floor_map[0]) and (xx != x or yy != y):
...
if __name__ == '__main__':
    ...
    input_data = [list(line) for line in advent_utils.load_input_from_file('inputs/test_11.txt')]
