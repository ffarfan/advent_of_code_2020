...
def puzzle_1(input_data):
    ...
    earliest_arrival_time = 100000000
    earliest_arriving_bus = 0
    ...
    for bus in buses:
        try:
            ...
            bus_arrival = math.ceil(arrival_time / bus) * bus
            if bus_arrival < earliest_arrival_time:
                earliest_arrival_time = bus_arrival
                earliest_arriving_bus = bus
        except ValueError:
            pass  # Not a bus

    wait_time = (earliest_arrival_time - arrival_time)

    return (wait_time * earliest_arriving_bus)


def puzzle_2(input_data):
    bus_data = [int(x) if is_int(x) else None for x in input_data[1].split(',')]
    buses = tuple((i, b) for i, b in enumerate(bus_data) if isinstance(b, int))

    step = 1
    next_step = 0

    for i, b in buses:
        next_step = next(c for c in count(next_step, step) if (c + i) % b == 0)
        step *= b
...
if __name__ == '__main__':
    input_data = advent_utils.load_input_from_file('inputs/input_13.txt')
    # input_data = advent_utils.load_input_from_file('inputs/test_13.txt')
